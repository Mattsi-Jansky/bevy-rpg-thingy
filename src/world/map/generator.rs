...
pub fn generate_map() -> Map {
    ...
    for _ in 0..num_rooms {
        let room_shape = room_shapes.get(room_shapes_distribution.sample(&mut rng)).unwrap();
        let max_x = DUNGEON_SIZE_X - room_shape.width - 1;
        let max_z = DUNGEON_SIZE_Z - room_shape.height - 1;
        if max_x <= room_shape.width || max_z <= room_shape.height {continue;}
        let location_distribution_x = Uniform::new(0,max_x);
        let location_distribution_z = Uniform::new(0,max_z);
        let point = MapPoint::new(location_distribution_x.sample(&mut rng),location_distribution_z.sample(&mut rng));
        possible_spawn_points.push(MapPoint::new(point
                                                     .x + Uniform::new(0,room_shape.width).sample(&mut rng),
    ...
    Map {tiles, spawn_point: possible_spawn_points.get(0).unwrap().clone()}
}
...
impl Room {
    ...
    pub fn new(north_west_point: MapPoint, width: usize, height: usize) -> Self {
        Self { north_west_point, width, height }
    }
