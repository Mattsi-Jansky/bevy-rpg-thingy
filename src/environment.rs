use bevy::prelude::{Commands, default, Res, Scene, SceneBundle, Transform};
use bevy::asset::{AssetServer, Handle};
use bevy::math::Quat;
use rand::distributions::{Uniform, Distribution};
use rand::prelude::ThreadRng;
use crate::{Tile, TILE_SIZE, TileType, WallType};
...
fn render_tile(commands: &mut Commands, meshes: &Res<Meshes>, mut rng: &mut ThreadRng, distribution: Uniform<i32>, x: usize, z: usize, tile: &Tile) {
    let maybe_tile_scene = match tile.tile_type {
        TileType::Dirt => {
            let random = distribution.sample(&mut rng);
            Some(
                if random == 0 { meshes.floor_dirt_a() } else { meshes.floor_dirt_b() }
            )
        }
...
fn render_wall_corners(commands: &mut Commands, meshes: &Res<Meshes>, x: usize, z: usize, tile: &Tile) {
